...
const startRecursive = async () => {
  ...
  let total = 0;
  while (page<=10) {
    const smallUsersArray = users.slice((size * (page -  1 )),(size*page));
    const result = await Promise.all(smallUsersArray.map (async(item)=> {
      return await processUser(item.id)
    }));
    const smalltotal = result.reduce((a, b) => {
      return a + b;
    });
